...
```
git add B.java
git commit -am "add message and operation"
```
...
```
Foi repetido o coamndo git merge math onde foi mostrado os conflitos em um arquivo onde havim marcadores <<<<<<< HEAD, ============= ,>>>>>>> que separam as suas modificações na HEAD das de outra branch esse arquivo foi alterado para acomodar ambas as alterações .
```

12. Escreva um comando (ou conjunto de comandos) para prosseguir com a mesclagem e atualizar o branch `master`
```
git add B.java
git commit -m "fix conflitos"
```
